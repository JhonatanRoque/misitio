...
class WPB_YourChannel{
	...
	public function loadDashJs($hook){
		if($hook === 'toplevel_page_yourchannel'){
	...
	static function instanceOutput($channel, $feed, $uniqid, $is_single){
		...
		$terms['form'] = array_merge(self::$terms['form'], $terms['form']);		
	...
	**/
	...
	public static function translateTerms(){ 
		...
		self::$terms['form'] = array(
			...
			'consent_statement' => __('Allow cookies?', 'YourChannel'),
			'consent_button' => __('Allow', 'YourChannel'),
			'consent_privacy_policy' => __('Privacy policy', 'YourChannel')
		);
